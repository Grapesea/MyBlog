...
> ##### WARNING
>
> <center><img src="..\photos\ctf\warning.png" style="zoom: 42%;"/></center>
> 
{: .block-warning }
...
>
> 由于本人计算机知识和素养极为匮乏，所以看到有问题的部分欢迎指正！
...
1. **支付漏洞 eg**
   ...
   <center><img src="..\photos\ctf\pay1.png" style = "zoom: 20%;"/></center>

   <center><img src="..\photos\ctf\pay2.png" style = "zoom: 25%;"/></center>

   <center><img src="..\photos\ctf\pay3.png" style = "zoom: 25%;"/></center>
...
5. XSS
   ...
   存储型XSS：数据被存储在服务器上，用户访问服务数据被自动投送/触发
...
* 参数污染
  ...
  <center><img src="..\photos\ctf\para_pollu1.png" style="zoom: 50%;" /></center>
  ...
  <center><img src="..\photos\ctf\para_pollu2.png" style="zoom: 40%;" /></center>
...
<center><img src="..\photos\ctf\ida_pseudo.png" style="zoom: 50%;" /></center>

右键点击参数并找到Rename variable：修改变量名

<center><img src="..\photos\ctf\ida_rename.png" style="zoom: 50%;" /></center>
...
<center><img src="..\photos\ctf\eg3-0.png" style="zoom: 33%;" /></center>

<center><img src="..\photos\ctf\eg3-1.png" style="zoom: 33%;" /></center>
...
__int64 __fastcall sub_401570(const char *table, _QWORD *a2, int *a3)
{
  ...
  do
  {
    newptr += 4;
    v13 = table[gap];
    *(newptr - 4) = ::table[v13 >> 2];
    v14 = table[gap + 1];
    *(newptr - 3) = ::table[(v14 >> 4) | (16 * v13) & 0x30];  // & 00110000
    v15 = table[gap + 2];
    gap += 3;
    *(newptr - 2) = ::table[(v15 >> 6) | (4 * v14) & 0x3C];   // & 00111100
    *(newptr - 1) = ::table[v15 & 0x3F];      //  & 00111111
  }
  while ( len > (int)gap );
LABEL_5:
  if ( r == 1 )
...
* **栈：从高地址往低地址生长。**
  ...
  <center><img src="..\photos\ctf\stack.png" style="zoom: 50%;" /></center>
...
<center><img src="..\photos\ctf\stack1.png" style="zoom: 50%;" /></center>

stack overflow的原理：程序向某个变量写进去的字节数超过了该变量本身申请的字节数，导致上方相邻的数据也被修改了。

<center><img src="..\photos\ctf\stack2.png" style="zoom: 50%;" /></center>
...
<center><img src="..\photos\ctf\hexedi.png" style="zoom: 50%;" /></center>
...
<center><img src="..\photos\ctf\jpeg.png" style="zoom: 25%;" /></center>
...
<center><img src="..\photos\ctf\png.png" style="zoom: 25%;" /></center>
...
## Misc Lec 2 OSINT（**Das Schloß**）

> 比较有意思的一节课，但知识点比较散，没怎么记笔记（全实操社工去了×）
...
[hackergame2022：旅行照片 2.0](https://github.com/USTC-Hackergame/hackergame2022-writeups/blob/master/official/旅行照片 2.0/README.md)
