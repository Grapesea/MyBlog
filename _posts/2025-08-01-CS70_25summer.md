...
* A. Termination 
  ...
  我一开始有一个疑惑，为什么不是$n^2-n$步之内？原因是算法并不知道要在哪里停止！考虑情况：
...
* G.P-R Algorithm is job-optimal





* one-set instance is not stable and two-set is stable.

  这里区分了两种不同的匹配问题，并说明了稳定匹配在这两种情况下的可行性：

  Two-set instance（两集合实例）：是指**两个不同集合之间的匹配**，比如：

  * 工作 ↔ 候选人
  * 医院 ↔ 住院医师
  * 学校 ↔ 学生
  * 男性 ↔ 女性（经典的stable marriage problem）

  **为什么总是存在稳定匹配？**

  * Gale-Shapley算法保证能找到稳定匹配
  * PDF中证明了这个算法总是终止且产生稳定结果
